    ...
    public function changeStatus() {
        if (isset($this->request->get['order_id']) && isset($this->request->get['status_id'])) {
            ...
            if ($status_id == NEW_ORDER_STATUS_ID) {
                ...
                $bcFile = $_SERVER["DOCUMENT_ROOT"] . "/admin/controller/bonusclub/api.php";
                if (file_exists($bcFile)) {
                    require $bcFile;

                    $this->load->libraryModel('extra/order');
                    $p_oprder = $this->model_extra_order->getOrder($this->request->get['order_id']);

                    global $registry;
                    $bcObj = new ControllerBonusclubApi($registry);
                    $bcObj->double($p_oprder);
                }
...
//            что после переезда считает не так
            $data = array(
//				'from' => COMPANY_ORIGIN,
                'from' => 'Шевченко, 1, Калининград,  Калининградская область, Россия',
                'to' => $to
            );
